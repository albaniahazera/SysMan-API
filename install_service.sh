...
echo "${APP_USER} ALL=(ALL) NOPASSWD: /sbin/shutdown, /sbin/reboot, /usr/bin/apt update, /usr/bin/apt upgrade, /usr/bin/systemctl restart nginx, /usr/bin/systemctl restart jellyfin" | sudo tee /etc/sudoers.d/bot_server_config > /dev/null
